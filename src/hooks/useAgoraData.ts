...
// v1.0.8 - Updated the share URL to be a permanent link to the GitHub project page.
...
export const useAgoraData = (userPublicKey: string) => {
        ...
        return new Promise((resolve, reject) => {
            ...
            RELAYS.forEach((relayUrl, index) => {
                ...
                const timeoutId = setTimeout(() => {
                    console.warn(`⏱️ Timeout for ${relayUrl} during event fetch.`);
                    socket.close();
                    completedSockets++;
        ...
        for (const relayUrl of RELAYS) {
            try {
                await new Promise<void>((resolve) => {
                    const socket = new WebSocket(relayUrl);
                    const timeoutId = setTimeout(() => { console.log(`⏱️ Timeout for ${relayUrl} during comment fetch.`); socket.close(); resolve(); }, TIMEOUT_MS);
                    ...
                    socket.onmessage = (msg) => {
                        ...
                        if (data[0] === 'EVENT' && data[2] && data[2].kind === 1) {
                            const comment = data[2]; const replyToId = comment.tags.find(t => t[0] === 'e')?.[1];
            ...
            for (const relayUrl of RELAYS) {
                try {
                    await new Promise<void>((resolve, reject) => {
                        const socket = new WebSocket(relayUrl);
                        const timeoutId = setTimeout(() => { console.log(`⏱️ Timeout for ${relayUrl} during profile fetch for ${pubkey.slice(0,8)}.`); socket.close(); reject(); }, TIMEOUT_MS);
    ...
    const fetchMyFriendsListWithTimeout = async () => {
        ...
        for (const relayUrl of RELAYS) {
            try {
                await new Promise<void>((resolve, reject) => {
                    const socket = new WebSocket(relayUrl);
                    const timeoutId = setTimeout(() => { console.warn(`⏱️ Timeout for ${relayUrl} during contact fetch.`); socket.close(); reject(); }, TIMEOUT_MS);
        ...
        for (const pubkey of myFollows) {
            ...
            for (const relayUrl of RELAYS) {
                try {
                    await new Promise<void>((resolve) => {
                        const socket = new WebSocket(relayUrl);
                        const timeoutId = setTimeout(() => { console.log(`⏱️ Timeout for ${relayUrl} during mutual check for ${pubkey.slice(0,8)}.`); socket.close(); resolve(); }, TIMEOUT_MS);
    ...
    const fetchRSVPsAndCancellations = async (eventIds: string[]) => {
        ...
        for (const relayUrl of RELAYS) {
            try {
                await new Promise<void>((resolve) => {
                    const socket = new WebSocket(relayUrl);
                    const timeoutId = setTimeout(() => { console.log(`⏱️ Timeout for ${relayUrl} during RSVP fetch.`); socket.close(); resolve(); }, TIMEOUT_MS);
                    ...
                    socket.onmessage = (msg) => {
                        ...
                        if (data[0] === 'EVENT' && data[2] && data[2].kind === 7) {
                            const eventId = data[2].tags.find(t => t[0] === 'e')?.[1]; const pubkey = data[2].pubkey; const content = data[2].content;
        ...
        for (const pubkey of hostPubkeys) {
            ...
            for (const relayUrl of RELAYS) {
                ...
                try {
                    await new Promise<void>((resolve) => {
                        const socket = new WebSocket(relayUrl);
                        const timeoutId = setTimeout(() => { console.log(`⏱️ Timeout for ${relayUrl} during profile fetch for ${pubkey.slice(0,8)}.`); socket.close(); resolve(); }, TIMEOUT_MS);
                        socket.onopen = () => { clearTimeout(timeoutId); socket.send(JSON.stringify(["REQ", `profile-${pubkey.substring(0,8)}`, { kinds: [0], authors: [pubkey], limit: 1 }])); };
                        socket.onmessage = (msg) => {
                            const data = JSON.parse(msg.data);
                            if (data[0] === 'EVENT' && data[2] && data[2].kind === 0) {
    ...
    const handleCreateEvent = async (e: React.FormEvent, newEvent: { name: string, location: string, startTime: string, description: string, image_url: string }) => {
        e.preventDefault();
        const storedPrivateKey = localStorage.getItem('agora_privateKey'); let signedEvent;
        if (storedPrivateKey) { signedEvent = nostrTools.finalizeEvent({ pubkey: userPublicKey, created_at: Math.floor(Date.now() / 1000), kind: 1, tags: [["t", "agora-meetup"]], content: JSON.stringify(newEvent), }, storedPrivateKey); }
        else if (window.nostr) { signedEvent = await window.nostr.signEvent({ created_at: Math.floor(Date.now() / 1000), kind: 1, tags: [["t", "agora-meetup"]], content: JSON.stringify(newEvent), }); }
    ...
    const handleToggleRSVP = async (eventId: string) => {
        ...
        const storedPrivateKey = localStorage.getItem('agora_privateKey'); let signedEvent;
        if (storedPrivateKey) { signedEvent = nostrTools.finalizeEvent({ pubkey: userPublicKey, created_at: Math.floor(Date.now() / 1000), kind: 7, tags: [['e', eventId], ['p', userPublicKey]], content: action }, storedPrivateKey); }
        else if (window.nostr) { signedEvent = await window.nostr.signEvent({ created_at: Math.floor(Date.now() / 1000), kind: 7, tags: [['e', eventId], ['p', userPublicKey]], content: action }); }
    ...
    const handlePostComment = async (eventId: string, finalContent: string) => {
        ...
        const storedPrivateKey = localStorage.getItem('agora_privateKey'); let signedEvent;
        if (storedPrivateKey) { signedEvent = nostrTools.finalizeEvent({ pubkey: userPublicKey, created_at: Math.floor(Date.now() / 1000), kind: 1, tags: [['e', eventId], ['p', userPublicKey]], content: finalContent }, storedPrivateKey); }
        else if (window.nostr) { signedEvent = await window.nostr.signEvent({ created_at: Math.floor(Date.now() / 1000), kind: 1, tags: [['e', eventId], ['p', userPublicKey]], content: finalContent }); }
        else { alert('Cannot sign action.'); return; }
        for (const relayUrl of RELAYS) { try { await new Promise<void>((resolve) => { const socket = new WebSocket(relayUrl); socket.onopen = () => { socket.send(JSON.stringify(["EVENT", signedEvent])); setTimeout(() => { socket.close(); resolve(); }, 500); }; socket.onerror = () => { socket.close(); resolve(); }; }); } catch (e) { console.warn(e); } }
        window.location.reload();
    };
    ...
    const handleShareAsNote = async (event: any) => {
        ...
        // Start building the note content
        let noteContent = `🗓️ EVENT: ${eventData.name}
📍 WHERE: ${eventData.location}
🕖 WHEN: ${startTime.toLocaleString(DateTime.DATETIME_FULL)}
...
Learn more and contribute on GitHub: https://github.com/Neopatriarch/AgoraMarketplace`;
        ...
        const storedPrivateKey = localStorage.getItem('agora_privateKey'); let signedEvent;
        if (storedPrivateKey) { signedEvent = nostrTools.finalizeEvent({ pubkey: userPublicKey, created_at: Math.floor(Date.now() / 1000), kind: 1, tags: [], content: noteContent, }, storedPrivateKey); }
        else if (window.nostr) { signedEvent = await window.nostr.signEvent({ created_at: Math.floor(Date.now() / 1000), kind: 1, tags: [], content: noteContent, }); }
        else { alert('No signing method available. Please log in again.'); return; }
    ...
    const handleShareEvent = async (event: any) => {
        ...
        // --- FINAL SOLUTION: Use a permanent, human-readable URL to the GitHub project ---
        const shareUrl = `https://github.com/Neopatriarch/AgoraMarketplace`;

        // --- Improved Text Formatting for Clipboard ---
        const textToCopy = `🗓️ ${eventData.name}
...
${shareUrl}`;
        ...
        if (navigator.share) {
            try {
                // For native share, we keep the original compact format
                const shareText = `🗓️ ${eventData.name}\n📍 ${eventData.location}\n🕖 ${startTime.toLocaleString(DateTime.DATETIME_FULL)}\n\n${eventData.description}`;
